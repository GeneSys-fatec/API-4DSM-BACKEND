...
export class ParameterLimitsController {
  async create(req: FastifyRequest, reply: FastifyReply) {
    ...
    async updateClimateRiskLimits(req: FastifyRequest<{ Params: { id: string }; Body: { minLimit: number; maxLimit: number } }>, reply: FastifyReply) {
        try {
            const id = parseInt(req.params.id);
            const { minLimit, maxLimit } = req.body;
            const updatedLimit = await service.updateLimit(id, minLimit, maxLimit);
            return reply.status(200).send(updatedLimit);
        } catch (error) {
            return reply.status(500).send({ error: "Erro ao atualizar os limites de risco climático" });
        }
    }
