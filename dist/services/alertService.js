...
export class AlertService {
    alertRepository = AppDataSource.getRepository(AlertLogEntity);
    measurementRepository = AppDataSource.getRepository(MeasurementEntity);
    parameterRepository = AppDataSource.getRepository(ParameterEntity);
    parameterLimitsRepository = AppDataSource.getRepository(parameterLimitsEntity);
    parameterTypeRepository = AppDataSource.getRepository(parameterTypeEntity);
    buildAutomaticMessage(parameterType, isBelowMin, measuredValue, limitValue) {
        ...
        const hasToken = (...tokens) => tokens.some((token) => key.includes(token) || name.includes(token));
    ...
    async findParameterById(parameterId) {
    ...
    async evaluateMeasurement(data) {
        const parameter = await this.findParameterById(data.parameterId);
        const occurredAt = this.toDate(data.occurredAt);
        const measurement = this.measurementRepository.create({
            idParameter: parameter,
        ...
        const message = this.buildAutomaticMessage(parameterType, isBelowMin, data.measuredValue, appliedLimit);
        const alert = this.alertRepository.create({
            idParameter: parameter,
            idMeasurement: savedMeasurement,
            triggeredValue: data.measuredValue,
